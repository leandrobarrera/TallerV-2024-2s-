...
/**
  * GPIO bit set an bit RESET enumeration
 */
enum
{
	GPIO_PIN_RESET    = 0  ,
...
/* Valores estandar para la configuracion */
/* 8.4.1 GPIx_MODER (dos bit por cada PIN) */
enum
{
	GPIO_MODE_IN   = 0,
	GPIO_MODE_OUT,
...
/* Structure definition which will keeps the configuration information por PinX */
typedef struct
{
	...
	uint8_t GPIO_PinPuPdControl; 			// Tunr ON-OFF the pull up and pull down resistor for working pin
	uint8_t GPIO_PinOutputType; 			// Select output type: push-pull or openDrain
	uint8_t GPIO_PinAltFunMode; 			// Type of alternate function for working pin.
...
typedef struct
{
	...
	GPIO_PinConfig_t 	pinConfig;	// Hold the configuration for the selected PinX.
	} GPIO_Handler_t ;
